...
操作步骤均给出可复现的完整命令与对应输出。以下所有终端截图均来自
\textbf{Ubuntu~22.04 虚拟机}（\$ \texttt{/system\_dev\_exp} 目录、\texttt{bash} 提示符、
Python~3.10），为个人实际执行的\emph{真实运行结果}，用于佐证命令输出与结论。
...
\texttt{python3 -m build} 从源码生成 wheel；随后新建一个干净虚拟环境，\textbf{仅
...
\texttt{24070030103}（见图 \ref{fig:q09-src}）。
\begin{lstlisting}[language=bash]
mkdir -p src/greetlab
cat > src/greetlab/__init__.py
cat > src/greetlab/cli.py
cat > pyproject.toml
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{q09_img0_src.png}
\caption{创建 src 布局：\texttt{__init__.py}、\texttt{cli.py} 与 \texttt{pyproject.toml}}
\label{fig:q09-src}
\end{figure}
\texttt{cli.py} 的入口与 \texttt{pyproject.toml} 关键内容：
\begin{lstlisting}[caption=cli.py 与 pyproject.toml]
# src/greetlab/cli.py
import argparse
def main():
...
\item \textbf{运行 \texttt{python3 -m build} 生成 wheel}（见图
\ref{fig:q09-build}）：
\begin{lstlisting}[language=bash]
cd q09
python3 -m build
...
\caption{python3 -m build 一次产出 sdist（.tar.gz）与 wheel（.whl）两种产物}
\label{fig:q09-build}
\end{figure}

\item \textbf{新建干净虚拟环境，仅从 wheel 安装}（见图 \ref{fig:q09-install}）：
用 \texttt{--no-index} + \texttt{--find-links} 强制只从本地 \texttt{dist/}
目录安装，杜绝联网拉包。
\begin{lstlisting}[language=bash]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q09_img2_install.png}
\caption{全新 venv，仅从本地 dist 安装 greetlab-24070030103，中断任何联网}
\label{fig:q09-install}
\end{figure}

\item \textbf{切换到 q09 之外运行}（见图 \ref{fig:q09-run}）：回到仓库根目录
（\texttt{q09} 之外），直接调用已安装到 venv 的 \texttt{sdt-greet} 脚本。
\begin{lstlisting}[language=bash]
cd ..                       # 离开 q09
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q09_img3_run.png}
\caption{在 q09 之外调用已安装的命令，中文姓名同样正确输出}
\label{fig:q09-run}
\end{figure}

\item \textbf{用 \texttt{pip3 show} 核对已安装信息}（见图
\ref{fig:q09-pipshow}）：确认包确实是 0.1.0、\texttt{Requires} 为空、位置在
\texttt{clean-venv/lib/python3.10/site-packages}。
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q09_img4_pipshow.png}
\caption{pip3 show 显示 greetlab-24070030103 version 0.1.0、Requires 为空}
\label{fig:q09-pipshow}
\end{figure}
\end{enumerate}

\subsection{运行结果}
\begin{itemize}
\item \texttt{python3 -m build} 输出
...
\item \texttt{python3 -m build} 会先在隔离环境安装 \texttt{setuptools>=68} 再构建，
...
\subsection{操作步骤}
\begin{enumerate}
\item \textbf{复制 q09 为 q10 并添加失败测试}：用 \texttt{cp -r} 复制源码，
新增 \texttt{tests/test\_cli.py}，用 \texttt{monkeypatch} 注入 \texttt{sys.argv}，
断言空白姓名抛出 \texttt{SystemExit} 且返回码为 2（见图
\ref{fig:q10-testfile}）。
\begin{lstlisting}[language=bash]
cp -r q09/* q10/
cd q10
mkdir tests
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q10_img0_testfile.png}
\caption{复制源码并创建失败测试 tests/test_cli.py}
...
\item \textbf{先运行测试，确认失败（红灯）}（见图 \ref{fig:q10-fail}）：
\begin{lstlisting}[language=bash]
...
\item \textbf{人工检查 diff 并复跑（绿灯）}（见图 \ref{fig:q10-pass}）：用
\texttt{git diff} 确认智能体仅新增两行、无无关改动，再复跑测试。
\begin{lstlisting}[language=bash]
...
\caption{智能体修改后的 cli.py 与复跑结果：1 passed}
\label{fig:q10-pass}
\end{figure}

\item \textbf{记录到 \texttt{ai\_log.md}}（见图 \ref{fig:q10-ailog}）：用
\texttt{cat} 写入不超过 5 行的记录（核心提示、智能体改动、人工验证）。
\begin{lstlisting}[language=bash]
cat > ai_log.md
# 1. 目标：--name 传入纯空白字符时程序以 SystemExit(2) 退出。
# 2. 约束：保留 argparse，使用 p.error() 完成退出。
# 3. 测试命令：PYTHONPATH=src pytest tests/test_cli.py -v
# 4. 智能体改动：增加 if not a.name.strip(): p.error(...) 两行。
# 5. 人工校验：git diff 确认无无关修改，测试由红灯转为绿灯。
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q10_img3_ailog.png}
\caption{git diff 确认改动范围，并记录 ai_log.md 的 5 行内容}
\label{fig:q10-ailog}
\end{figure}
\end{enumerate}
...
\item 修复后：\texttt{test\_blank\_name\_exits\_code\_2 PASSED}，
\texttt{1 passed}。
...
用 \texttt{cat} 将改写后的成品写入 \texttt{communication.md} 并回显（见
图 \ref{fig:q11-comm}）。
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{q11_img1_comm.png}
\caption{写入并回显 communication.md：Issue / 提交信息 / 评审意见}
\label{fig:q11-comm}
\end{figure}
...
\subsection{操作步骤}
\begin{enumerate}
\item \textbf{创建并补全 \texttt{train.py}}（见图 \ref{fig:q12-src}）：数据为
$y = 3x - 1$ 的线性关系（\texttt{x = torch.linspace(-1, 1, 100)}），模型为单层
\texttt{nn.Linear(1, 1)}，损失用 \texttt{MSE}，优化器用 \texttt{SGD(lr=0.1)}，
训练 200 轮。
\begin{lstlisting}[language=bash]
cat > train.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{q12_img0_src.png}
\caption{写入完整 train.py：含补全的训练循环与评估打印}
\label{fig:q12-src}
\end{figure}
...
\item \textbf{运行并观察收敛}（见图 \ref{fig:q12-train}）：
\begin{lstlisting}[language=bash]
python3 train.py
\end{lstlisting}
\begin{figure}[H]
...
\caption{训练 200 轮后 final loss = 0.000000，weight = 2.999997，bias = -1.000000}
\label{fig:q12-train}
\end{figure}
\end{enumerate}
...
\end{lstlisting}
实际运行先输出一条 \texttt{UserWarning}（提示未安装 \texttt{numpy}，这是 PyTorch
内部可选依赖，不影响本次 CPU 上的线性回归），随后打印最终损失 0.000000、
weight 2.999997、bias $-1.000000$。最终损失远小于 0.001，与数据生成式
$y = 3x - 1$ 完全一致，且全程未手工赋值参数，靠梯度下降自然收敛。
...
\item 数据生成式 $y = 3x - 1$ 为确定性线性关系，且模型仅一个可学习层，故即使
不设随机种子也能稳定收敛到 weight \textasciitilde{}3、bias \textasciitilde{}$-1$；
若需严格可复现，可在建模前追加 \texttt{torch.manual\_seed(...)}（第 2 周 q08
「先建立可重复基线」的思路）。
\end{itemize}
...
\end{itemize}
仓库主页见图 \ref{fig:github-page}：页面顶部为 \texttt{main} 分支、分支/标签数量、
提交总数（\textbf{20 次提交}）与最新一次提交记录；下方为完整文件清单，每个文件
右侧标注最近一次改动它的 commit 信息，可直观验证「分次提交」确实按题号落地。
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{github_repo_page.png}
\caption{GitHub 仓库主页 yingjunzhu520/system-tools-hamesome-3（main 分支、20 次提交、文件清单）}
\label{fig:github-page}
\end{figure}

\subsection{提交记录（分次提交，非全量）}
本地仓库按「初始化模板 $\to$ q09 $\to$ q10 $\to$ q11 $\to$ q12 $\to$ 截图脚本
$\to$ 报告撰写 $\to$ README $\to$ PDF 预览 $\to$ 提交记录截图
$\to$ 替换/新增真实截图 $\to$ 重写报告嵌入真实截图 $\to$ 更新 PDF 预览
$\to$ 写入 GitHub 仓库主页截图 $\to$ 同步提交记录」
的顺序分次提交，每步都是增量修改（无单次全量提交）。\texttt{git log --oneline}
实际输出如下（由新到旧）：

\begin{lstlisting}[language=bash]
d50fffc chore: refresh commit screenshot to latest history
d98a91a docs(report): sync git log listing to latest history
c059c24 chore(preview): render GitHub page screenshot in fpdf2 PDF preview
8d4b1a2 docs(report): embed GitHub repo page screenshot in version-control section
6558893 feat: add real GitHub repository page screenshot
aa3a391 chore: refresh commit screenshot to final 19-commit history
75a50d1 docs(report): refresh version-control section with 16-commit history
f4c6be6 chore(preview): embed real screenshots in fpdf2 PDF preview (10 pages)
b376ee1 docs(report): embed 12 real screenshots and align narrative with Linux VM
1f4b1dc feat: add 8 real terminal screenshots for q09-q12
2d9076d chore: replace 4 script-rendered screenshots with real terminal captures
c8ae8f3 docs: add figure cross-references in report body
d4b7c91 docs: finalize commit history record in report
...
各题一一对应且互不合并，满足「禁止单次全量提交」的要求。仓库主页外观见
图 \ref{fig:github-page}，提交历史的整理视图见图 \ref{fig:commit}。
...
\caption{GitHub 仓库提交记录整理视图（main 分支，按仓库真实提交数据绘制）}
\label{fig:commit}
\end{figure}
...
\begin{itemize}
  \item \textbf{q09（打包与交付）}：掌握 \texttt{src} 布局 +
        \texttt{pyproject.toml} + \texttt{python3 -m build} 的完整打包链路，
