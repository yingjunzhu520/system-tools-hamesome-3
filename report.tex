...
\begin{lstlisting}[language=toml, caption=pyproject.toml]
[build-system]
...
\textbf{改写后（可执行）：}
\begin{lstlisting}[language=markdown, caption=communication.md（节选）]
## Issue
...
$\to$ 报告撰写 $\to$ README $\to$ 提交记录截图」的顺序分次提交，每步都是增量
修改（无单次全量提交）。\texttt{git log --oneline} 实际输出见下文（由新到旧）。
