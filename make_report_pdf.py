...
body("报告每个实验按「任务说明 -> 操作步骤 -> 运行结果 -> 要点」组织；"
     "运行结果均来自 Windows + Git Bash 沙箱中的真实执行。")

# ============ 2 第 9 题 ============
section("2", "第 9 题：从源码构建并在干净环境安装 Wheel")
sub("任务说明")
body("在 q09 中创建最小 Python 命令行包 greetlab，用 python -m build 从源码生成 "
     "wheel；新建干净虚拟环境，仅从 wheel 安装（不联网），并切换到 q09 之外运行 "
     "sdt-greet --name <学号> 验证输出。")
sub("操作步骤")
body("1) 创建 src 布局：src/greetlab/__init__.py + cli.py，并补齐 pyproject.toml，"
     "把「学号」替换为 24070030103。")
...
body("2) 运行 python -m build 生成 wheel：")
...
sub("操作步骤")
body("1) 添加 tests/test_cli.py（monkeypatch 注入 sys.argv）：")
code("def test_blank_name_exits_code_2(monkeypatch):\n"
...
code("$ PYTHONPATH=src pytest tests/test_cli.py -v\n2 passed in 0.07s")
body("ai_log.md 共 5 行，记录核心提示、智能体改动与人工验证。")
sub("要点")
...
sub("操作步骤")
body("数据 y = 3x - 1，模型 nn.Linear(1,1)，损失 MSE，优化器 SGD(lr=0.1)，训练 200 轮：")
...
body("最终损失 < 0.001，weight 收敛到 2.999997、bias 收敛到 -1.000000，与生成式 y = 3x - 1 一致。")
sub("要点")
...
pdf.output(r"C:\Users\LENOVO\Desktop\系统开发工具基础_第3周_报告\report.pdf")
